...
try_again:
 ...
 end_transfer:
	...
	if (h2c->dff & H2_F_DATA_END_STREAM) {
		/* no more data are expected for this message */
		/* FIXME: Don't set it on the last tunneled data (not handled for now) */
		htx->flags |= HTX_FL_EOM;
	}
...
static size_t h2s_frt_make_resp_headers(struct h2s *h2s, struct htx *htx)
{
	...
	for (blk = htx_get_head_blk(htx); blk; blk = htx_get_next_blk(htx, blk)) {
		...
		if (type == HTX_BLK_RES_SL) {
			...
			if (!sl) {
				TRACE_ERROR("no start-lines", H2_EV_TX_FRAME|H2_EV_TX_HDR|H2_EV_H2S_ERR, h2c->conn, h2s);
				goto fail;
			}
 ...
 retry:
	...
	if ((htx->flags & HTX_FL_EOM) && htx_is_empty(htx) && h2s->status >= 200) {
		/* EOM+empty: we may need to add END_STREAM except for 1xx
		 * responses and 2xx responses to a connect.
		 */
...
static size_t h2s_make_data(struct h2s *h2s, struct buffer *buf, size_t count)
{
 ...
 send_empty:
	...
	if (fsize == bsize) {
		...
		if ((htx->flags & HTX_FL_EOM) && htx_is_empty(htx)) {
			/* EOM+empty: we may need to add END_STREAM */
			es_now = 1;
		}
	}
	else {
...
static size_t h2s_skip_data(struct h2s *h2s, struct buffer *buf, size_t count)
{
 ...
 next_data:
	...
	if (!(htx->flags & HTX_FL_EOM) || !htx_is_last_blk(htx, blk))
		goto skip_data;
	...
	while (h2s->st < H2_SS_HLOC && !(h2s->flags & H2_SF_BLK_ANY) &&
		...
		switch (btype) {
			...
			case HTX_BLK_DATA:
				...
				if (!(h2s->h2c->flags & H2_CF_IS_BACK) && (h2s->flags & H2_SF_BODYLESS_RESP))
